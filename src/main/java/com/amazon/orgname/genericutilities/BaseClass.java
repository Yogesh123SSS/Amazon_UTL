...
public class BaseClass {
	...
	public void afterConfiguration() {
	  ...
	  @BeforeMethod
	  @Parameters("browserName") 
	  public void beforeConfigurationOfGroup(String browserName) throws IOException, InterruptedException {
	  //Here data is reading and supplying from TestNG suite file parameterization
	  //String browserName = fileUtils.readDataFromPropertyFile("Browser");
	  if(browserName.equals("Chrome")) { ChromeOptions options=new ChromeOptions();
	  options.addArguments("--profile-directory=Default");
	  options.addArguments("--disable-blink-features=AutomationControlled");
	  options.addArguments("--disable-notifications"); driver = new
	  ChromeDriver(options); } else if( browserName.equals("Firefox")) { driver =
	  new FirefoxDriver(); } else if(browserName.equals("Edge")) { driver = new
	  EdgeDriver(); } driver.manage().window().maximize(); //Thread.sleep(30000);
	  driver.get(fileUtils.readDataFromPropertyFile("URL"));
	  
	  }
	  
	 @AfterMethod
	 public void afterConfigurationOfGroups()
	 {
